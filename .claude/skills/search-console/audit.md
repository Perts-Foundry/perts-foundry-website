...
- **Healthy**: the picker shows the Domain property `sc-domain:pertsfoundry.com`, confirmed live on
  2026-09-22 (Settings > Ownership verification read "You are a verified owner"). The picker's
  listbox rendered empty in the accessibility tree, so record the value only; never try to
  enumerate other properties.
...
- **Where**: URL inspection of the homepage (the header combobox) lists a referring page on a
  second brand host. The host is a constant, `SECONDARY_DOMAINS` (["www.pertsfoundry.com"]) in
  `lib/checks.mjs`, confirmed by a node probe on the port date (2026-09-22): `www.pertsfoundry.com`
  answers 301 to `https://pertsfoundry.com/`. `review.mjs` probes each host with one unfollowed node
  request, plus any host in the optional `settings.secondary_domains`.
...
- **Fix owner**: infrastructure repo (Cloudflare DNS, via Terraform).
- **Marker**: capture: settings

### 3. Ownership verification

- **Where**: `Settings` > `Ownership verification`, view `ownership`.
- **Healthy**: "You are a verified owner"; "Verification methods used" names "Domain name
  provider" (confirmed live on 2026-09-22).
- **Failure**: not verified; or verified only by an HTML tag or file, which a deploy can remove.
- **Checks**: `verification-lost` (`SEVERITY['verification-lost']`),
  `verification-method-fragile` (`SEVERITY['verification-method-fragile']`).
- **Fix owner**: infrastructure repo (re-verify by domain name provider, via Terraform DNS).
- **Marker**: capture: settings
...
- **Healthy**: "Associated services" lists Analytics; "Pending requests" is empty. The site sells
  nothing, so a Merchant Center association is out of scope by design and never checked.
- **Failure**: the Analytics association missing; a pending request nobody started.
...
- **Failure**: a new site is selected; Google is being told the site moved.
...
- **Failure**: "Exclude", which removes the site from AI Overviews and AI Mode traffic.
...
- **Healthy**: host status with no issues; 5xx share at or under `CRAWL_5XX_SHARE` (0.01); the
  ...
  out, at or under `CRAWL_4XX_SHARE` (0.05). "No data available yet" is host status no-data inside
  an ok report, so the robots.txt state from surface 9 is kept.
- **Failure**: host issues; a 5xx or 4xx share over its threshold (a Cloudflare bot-management block
  shows up here as 4xx).
...
- **Fix owner**: infrastructure repo (Cloudflare hosting, headers and bot-management settings, via
  Terraform).
- **Marker**: capture: crawl-stats
...
- **Where**: `User settings` in the header, view `user-settings`, rows `Email preferences` and
  ...
  `user-settings/performance-on-search` (the second is not judged).
...
- **Healthy**: "Submitted sitemaps" lists `https://pertsfoundry.com/sitemap.xml` once, Type
  "Sitemap" (Hugo emits a single `urlset`, never a sitemap index), Status "Success", Discovered
  pages within `SITEMAP_TOLERANCE` (1) of the live sitemap count, Last read within
  `SITEMAP_STALE_READ_DAYS` (14) days.
- **Failure**: no sitemap row; "Couldn't fetch" or "Has errors"; a discovered count far from live.
  `sitemap-child-submitted` stays registered for a non-index child someone submits by hand; it does
  not apply to Hugo's own output.
...
- **Fix owner**: GSC UI for submissions; Hugo generates the sitemap itself.
...
- **Healthy**: indexed count at or above the live sitemap count less its `NOINDEX_OK` paths (empty
  today: a real Hugo build emits no noindex meta tag anywhere); every "Why pages aren't indexed"
  row is expected. Record each reason with its count, its Source column (Website or Google systems)
  and up to five example URLs, query strings and fragments stripped, `/api/` routes dropped.
- **Failure**: indexed well below the sitemap; any noindex; server errors; a Google-chosen
  canonical that overrides the declared one; a reason label never seen before.
...
- **Fix owner**: repo (Hugo templates and content front matter for canonicals and robots meta;
  `layouts/partials/extend-head-uncached.html` for structured data).
...
- **Healthy**: the homepage, one published service, one published case study, one published
  small-business page and one published blog post, at most eight URLs per run, only pages with
  `draft: false` and a `publishDate` in the past. "URL is on Google"; user-declared and
  Google-selected canonicals match; last crawl within `STALE_CRAWL_DAYS` (30) days; crawl and
  indexing allowed; page fetch successful; discovered via Sitemaps; the "Enhancements & Experience"
  rows list the rich results `EXPECTED_RICH_RESULTS` expects for the path (services, case studies,
  small-business and blog pages: Breadcrumbs; the homepage: none).
- **Failure**: not on Google; a canonical mismatch; crawl or indexing blocked on a sitemap URL; a
  failed fetch; an expected rich result missing; a panel section never seen before.
...
- **Fix owner**: repo (Hugo templates, `layouts/partials/extend-head-uncached.html` for JSON-LD,
...
- **Fix owner**: repo (CSS modules and layout templates); the "Try PageSpeed Insights" links leave
  Search Console and are never followed.
- **Marker**: capture: cwv

### 18. HTTPS

- **Where**: `Experience` > `HTTPS`, view `https`, present in the left navigation and confirmed
  live on 2026-09-22.
...
- **Fix owner**: infrastructure repo (Cloudflare domains and TLS, via Terraform).
- **Marker**: capture: https

### 19. Enhancements

- **Where**: the `Enhancements` heading of the left navigation, present only once a rich-result
  type is detected. Confirmed live on 2026-09-22: `Breadcrumbs`, view `r/breadcrumbs`, is present;
  `Review snippets`, view `r/review-snippet`, is conditional (not yet observed on this property; the
  first live run records it). There is no `Shopping` heading and no Product snippets or Merchant
  listings item: the site sells nothing. Each report fills one of `items[]`: valid and invalid
  partition its items, `items[].warning` is the report's own "with warnings" figure or null when it
  shows none (never derived), and `items[].issues[]` holds each issue row's label, item count and
  level.
- **Healthy**: per type (Breadcrumbs, and any other), zero invalid and zero warning items.
- **Failure**: invalid or warning items; Breadcrumbs absent while an inspected page inspects as
  indexed; a type never seen before. While a section is absent the report is `not-present` and
  nothing is judged: an inspection lists rich results days before the section appears.
- **Checks**: `enhancement-invalid` (`SEVERITY['enhancement-invalid']`), `enhancement-warning`
  ...
  kept registered for a future overlap, for example two rich-result families both reporting items
  for the Product snippets and Merchant listings names, though the site has no catalog to trigger it
  today), `enhancement-type-unknown` (`SEVERITY['enhancement-type-unknown']`).
- **Fix owner**: repo (`layouts/partials/extend-head-uncached.html` for JSON-LD).
...
- **Fix owner**: repo (content front matter titles and descriptions,
  `layouts/partials/extend-head-uncached.html` fallbacks; new content via the `generate-blog`
  command).
...
- **Where**: `Indexing` > `Videos`, view `video-index`, present in the left navigation only once
  Google has found a video; not yet observed on this property (the live check on 2026-09-22 showed
  no Videos item). An absent item makes the report not-present.
- **Healthy**: every video Google found indexed, zero not indexed.
- **Failure**: videos Google found and did not index.
- **Checks**: `videos-not-indexed` (`SEVERITY['videos-not-indexed']`).
- **Fix owner**: repo (any embedded video markup on a content page).
