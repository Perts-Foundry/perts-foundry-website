...
5. `navigate_page` to `https://search.google.com/search-console/?resource_id=sc-domain%3Apertsfoundry.com`.
6. `take_snapshot`, then the URL check below. Confirm the property picker reads
   `pertsfoundry.com` (otherwise `gsc-property-mismatch`, stop).
...
- Any other host (PageSpeed Insights, support pages, the site itself, account pages): `close_page`
...
Every view URL is `https://search.google.com/search-console/<view>?resource_id=sc-domain%3Apertsfoundry.com`.
...
| 25 | `video-index`, when present | "Video indexing", "videos indexed" | `videos.indexed`, `videos.not_indexed` |
...
| 19 | `r/breadcrumbs`, `r/review-snippet`, and any other link under the Enhancements heading | "Valid", "Invalid", "Processing data" | `enhancements.items[]` |
...
4. Transcribe up to 5 URLs from the Examples table into that reason's `examples`: property host
   only, query string and fragment stripped, and skip any path under `/api/` (the schema refuses
   it anyway).
...
5. Page URLs lose their query strings and fragments. A URL under `/api/` (anywhere in the path), or
   with a token-shaped segment, is left out of the capture entirely.

## URL inspection

For each URL (the homepage, one published service page, one published case study, one published
small-business page and one published blog post, only pages with `draft: false` and a
`publishDate` in the past; never more than eight in a run, and never a URL picked from a captured
report):
...
`status: "not-ready"` for that report, with no data fields. A report whose navigation item is
absent (Videos, or an Enhancements type such as Review snippets that has not yet accumulated data)
is `status: "not-present"`. Record it and move on.
...
Videos and each Enhancements type are surfaces now, not discoveries. For a left-navigation item not
in `KNOWN_SURFACES`, navigate to it read-only, `take_snapshot`, and add an `unknown[]` entry
`{ kind, label, path, note }` with a note of at most 200 characters. Click nothing on that page.

## Writing the capture

Transcription comes from snapshots only. `take_screenshot` is used with no `filePath` (inline only):
the operating profile refuses any path outside its own temp directory, so a screenshot is never
saved to disk and never lands in the repo. Start from the `--template` skeleton and fill it:
