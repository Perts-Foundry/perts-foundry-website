...
name: search-console
description: >-
  Audit the Google Search Console setup for pertsfoundry.com and read its reports through the
  operator's logged-in browser, in an attended session: property type and verification, users,
  ...
  read. Requires an attended browser session; not for the Search Console API, Bing, GA4 or the
  Cloudflare dashboard, keyword tooling, or applying fixes.
...
Search Console is verified for pertsfoundry.com. This skill reads what Google made of the site,
audits the Search Console setup itself, and notices when Search Console grows a report or a
setting the skill has never seen.
...
"Ready to open Search Console for `sc-domain:pertsfoundry.com` in the MCP browser and read
...
- **Read-only, with one named exception.** Nothing writes to the site or the repo, and nothing
  changes in Search Console except that opening an indexing alert message marks it read. The only
  writes are the capture and the run files, both in the state dir outside the checkout.
...
- **Public repo.** No email address, user name, Google account, machine path, sub-state location,
  ...
  release note. Describe the class of a query, never the query. The property host and public site
  metadata are fine.
...
- **Navigation.** `https://search.google.com/search-console/*` only. `accounts.google.com` is the
  Login STOP. Every other host (PageSpeed Insights, support pages, the site itself, account pages)
...
- **The site is fetched by node** (`review.mjs` through `lib/sitemap.mjs`), never by the browser,
  and never with curl.
...
This skill does NOT: use the Search Console API; touch Bing, GA4 or the Cloudflare dashboard; do
keyword research; write SEO fields or edit template code; submit or delete sitemaps; request
indexing; validate fixes; request removals; associate services; change the Search generative AI
control; configure bulk export; start a change of address; add or remove users; remove the
property; mark a message unread; or edit its own files during a run. Fixes go through the normal PR
flow, never this skill.
