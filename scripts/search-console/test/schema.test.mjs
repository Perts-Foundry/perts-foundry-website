...
test('a token-shaped or Worker API URL is rejected; a long hyphenated slug is not', () => {
  const token = ['Z2NwLXVzLWVh', 'c3QxOjAxSkY'].join('');
  expectError((c, r) => { r['indexing-pages'].reasons = [{ reason: 'not-found', source: 'Website', count: 1, examples: [`${ORIGIN}/api/contact/${token}`] }]; r['indexing-pages'].not_indexed = 1; }, /examples\/0$/, /token-bearing/);
  expectError((c, r) => { r['indexing-pages'].reasons = [{ reason: 'not-found', source: 'Website', count: 1, examples: [`${ORIGIN}/blog/${token}`] }]; r['indexing-pages'].not_indexed = 1; }, /examples\/0$/, /token-bearing/);
  expectError((c, r) => { r.links.top_linked_pages[0].url = `${ORIGIN}/api/contact`; }, /top_linked_pages\/0\/url$/, /token-bearing/);
  assert.equal(isSensitivePath('/case-studies/enterprise-ai-tooling-adoption/'), false);
  assert.equal(isSensitivePath('/api/contact'), true);
  assert.equal(isSensitivePath('/api/contact/status'), true);
});

test('a URL with a query string or fragment, or off the property host, is rejected', () => {
  expectError((c, r) => { r.performance.pages.rows[0].url = `${ORIGIN}/?ref=1`; }, /pages\/rows\/0\/url$/, /query string/);
  expectError((c, r) => { r.performance.pages.rows[0].url = `${ORIGIN}/#top`; }, /pages\/rows\/0\/url$/, /fragment/);
  expectError((c, r) => { r.performance.pages.rows[0].url = 'https://other-example.test/'; }, /pages\/rows\/0\/url$/, /property host/);
  const c = baseCapture();
  c.reports.inspections.items[1].google_canonical = 'https://other-example.test/services/x/';
...
test('property host helpers', () => {
  assert.equal(propertyHost('sc-domain:pertsfoundry.com'), 'pertsfoundry.com');
  ...
  assert.equal(onPropertyHost('www.pertsfoundry.com', 'sc-domain:pertsfoundry.com'), true);
  assert.equal(onPropertyHost('www.example.test', 'https://example.test/'), false);
  assert.equal(onPropertyHost('evil-pertsfoundry.com', 'sc-domain:pertsfoundry.com'), false);
});
...
test('isSensitivePath: public site paths pass; token-shaped and Worker API routes do not', () => {
  const pass = [
    '/sitemap_products_1.xml', '/sitemap_pages_1.xml', '/sitemap_blog_1.xml',
    '/blog/ai-accelerated-terraform-migrations/', '/case-studies/agile-transformation-defense/', '/',
  ];
  const refuse = [
    '/api/contact', '/API/contact', '/api/contact/status', '/services/abcdef0123456789abcdef01/',
...
test('unread above total, more examples than the count, and an email-shaped key are rejected', () => {
  ...
  expectError((c, r) => {
    r['indexing-pages'].reasons = [{ reason: 'soft-404', source: null, count: 1, examples: [`${ORIGIN}/about/`, `${ORIGIN}/contact/`] }];
...
// ---------------------------------------------------------------------------------------------
// Enhancement issue rows, message alerts, anchor misses and videos.
...
test('message alerts: empty examples accepted; count, report status and URL shape enforced', () => {
  ...
  expectError((c2, r) => { r.messages.alerts = [alert({ examples: [`${ORIGIN}/about/?x=1`] })]; }, /alerts\/0\/examples\/0$/, /query string/);
  expectError((c2, r) => { r.messages.alerts = [alert({ examples: ['https://other-example.test/about/'] })]; }, /alerts\/0\/examples\/0$/, /not on the property host/);
});
...
test('videos: counts are non-negative; not-present carries none; not-ready may carry none for insights', () => {
  const c = baseCapture();
  c.reports.videos = { captured_at: c.reports.videos.captured_at, report: 'videos', status: 'ok', indexed: 0, not_indexed: -1 };
  assert.ok(errorsOf(c).some((e) => e.path === '/reports/videos/not_indexed'));
  const nr = baseCapture();
  nr.reports.videos = notReady('videos');
  assert.deepEqual(errorsOf(nr), []);
