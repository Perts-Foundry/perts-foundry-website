...
import { findingKey, exitCodeFor } from '../lib/shared/findings.mjs';
...
test('the pre-index fixture raises nothing above INFO', () => {
  ...
  const notReadyReports = findings.filter((f) => f.check === 'gsc-not-ready').map((f) => f.url).sort();
  assert.deepEqual(notReadyReports, ['indexing-pages', 'inspections', 'links', 'performance', 'videos']);
...
test('brand-only is vacuous on zero queries and on zero impressions', () => {
  ...
  assert.ok(isBrandQuery('Perts  Foundry devops consulting'));
  assert.ok(isBrandQuery('pertsfoundry'));
  assert.ok(!isBrandQuery('kubernetes consulting'));
});

test('NOINDEX_OK is empty today: no path is exempt', () => {
  assert.ok(!isNoindexOk('/tags/aws/'));
  assert.ok(!isNoindexOk('/404.html'));
  assert.ok(!isNoindexOk('/'));
});

test('the noindex reason: unexempt examples are ERROR, no examples leaves an unverified remainder', () => {
  const noindex = (count, paths) => (r) => {
    r['indexing-pages'].reasons[0] = { reason: 'noindex', source: 'Website', count, examples: paths.map((p) => `${ORIGIN}${p}`) };
  };
  const remainder = run('index-reason-noindex', noindex(3, []));
  ...
  const flagged = run('index-reason-noindex', noindex(2, ['/tags/aws/', '/404.html']));
  assert.deepEqual(flagged.map((f) => [f.severity, new URL(f.url).pathname]).sort(), [[ERROR, '/404.html'], [ERROR, '/tags/aws/']]);
});
...
test('not-found is WARN only for a URL still in the live sitemap', () => {
  ...
  assert.equal(run('index-reason-not-found', nf('/about/'))[0].severity, WARN);
  assert.equal(run('index-reason-not-found', nf('/retired-page/'))[0].severity, INFO);
});
...
test('removal-active is ERROR only for a URL in the live sitemap', () => {
  ...
  assert.equal(run('removal-active', removal('/about/'))[0].severity, ERROR);
  assert.equal(run('removal-active', removal('/retired-page/'))[0].severity, WARN);
  assert.equal(run('removal-active', (r) => { r.removals.temporary_active = 2; })[0].severity, WARN);
});

test('inspect-crawl-blocked skips a URL absent from the live sitemap', () => {
  assert.ok(!fires('inspect-crawl-blocked', (r) => {
    r.inspections.items[2].url = `${ORIGIN}/retired-page/`;
...
test('enhancement checks judge only a present report: not-present and not-ready stay quiet', () => {
  ...
  assert.ok(fires('enhancement-absent', (r) => { r.enhancements.items = []; }));
});
...
test('a partial live sitemap still proves the URLs it listed; absence and the count stay unknown', () => {
  const partial = (o) => {
    o.sitemapUrls = SITEMAP_URLS.filter((u) => !u.includes('/case-studies/'));
    o.sitemapState = 'partial';
  };
  const removal = (r) => { r.removals.temporary_active = 1; r.removals.temporary_urls = [`${ORIGIN}/about/`]; };
  assert.equal(run('removal-active', removal, partial)[0].severity, ERROR, 'a listed URL is proven present');
  const notFound = (r) => { r['indexing-pages'].reasons.push({ reason: 'not-found', source: 'Website', count: 1, examples: [`${ORIGIN}/about/`] }); };
  assert.equal(run('index-reason-not-found', notFound, partial)[0].severity, WARN, 'a listed URL is proven present even from a partial read');
  assert.ok(fires('inspect-crawl-blocked', (r) => { r.inspections.items[2].indexing_allowed = false; }, partial), 'a case study the partial read missed is not proven absent');
...
test('messages-alert-examples: one finding per alert, paths and reason in the detail', () => {
  const f = run('messages-alert-examples', (r) => {
    r.messages.total = 3;
    r.messages.alerts = [
      { subject: 'New reasons prevent pages from being indexed', examples: [`${ORIGIN}/about/`, `${ORIGIN}/retired-page/`], reason_label: "Excluded by 'noindex' tag" },
  ...
  assert.equal(f[0].detail, `indexing alert names 2 page(s): /about/, /retired-page/, reason "Excluded by 'noindex' tag"`);
...
test('an alert naming a reason with no examples appends to that reason only', () => {
  const setup = (reasonExamples, alertOver) => (r) => {
    ...
    r.messages.alerts = [{ subject: 'New reasons prevent pages from being indexed', examples: [`${ORIGIN}/about/`, `${ORIGIN}/retired-page/`], ...alertOver }];
  };
  ...
  assert.deepEqual(detail([`${ORIGIN}/contact/`], { reason_label: 'Soft 404' }), ['3 page(s), source Website']);
...
test('videos-not-indexed fires on a not-indexed count and never on a not-ready report', () => {
  assert.ok(fires('videos-not-indexed', (r) => { r.videos = { captured_at: r.videos.captured_at, report: 'videos', status: 'ok', indexed: 0, not_indexed: 2 }; }));
...
test('enhancement details carry issue labels, and are unchanged without them', () => {
  const two = run('enhancement-warning', (r) => {
    Object.assign(r.enhancements.items[0], { warning: 3, issues: [
      { label: "Missing field 'name'", items: 1, level: 'warning' },
      { label: "Missing field 'item'", items: 3, level: 'warning' },
      { label: "Missing field 'position'", items: 1, level: 'error' },
    ] });
  });
  assert.equal(two[0].detail, `3 item(s) with warnings: "Missing field 'item'" (3), "Missing field 'name'" (1)`);
  const invalid = run('enhancement-invalid', (r) => {
    Object.assign(r.enhancements.items[0], { invalid: 1, issues: [
      { label: "Missing field 'position'", items: 1, level: 'error' }, { label: "Missing field 'name'", items: 2, level: 'warning' },
    ] });
  });
  assert.equal(invalid[0].detail, `1 invalid item(s): "Missing field 'position'" (1)`);
  ...
  const unknown = run('enhancement-warning', (r) => {
    Object.assign(r.enhancements.items[0], { warning: null, issues: [{ label: "Missing field 'name'", items: 2, level: 'warning' }] });
  });
  assert.equal(unknown[0].detail, `unknown item(s) with warnings: "Missing field 'name'" (2)`);
...
test('the nav entries that are conditional: absent says nothing, present is known', () => {
  const conditional = ['Videos', 'HTTPS', 'Enhancements', 'Breadcrumbs', 'Review snippets'];
  for (const label of conditional) assert.ok(KNOWN_SURFACES.nav.find((n) => n.label === label)?.conditional, label);
  assert.ok(!fires('surface-gone', (r) => { r.discovery.nav = r.discovery.nav.filter((n) => !conditional.includes(n.label)); }));
  const withAll = (r) => {
...
const SUBJECT = 'New reasons prevent pages from being indexed on site pertsfoundry.com';

test('alerts sharing a subject each keep their own finding', () => {
  const f = run('messages-alert-examples', (r) => {
    r.messages.total = 3;
    r.messages.alerts = [
      { subject: SUBJECT, examples: [`${ORIGIN}/about/`] },
      { subject: SUBJECT, examples: [`${ORIGIN}/retired-page/`] },
      { subject: SUBJECT, examples: [] },
    ];
  });
  assert.deepEqual(f.map((x) => x.url), [SUBJECT, `${SUBJECT} #2`, `${SUBJECT} #3`]);
  assert.match(f[0].detail, /\/about\//);
  assert.match(f[1].detail, /\/retired-page\//);
});
...
test('an alert reason label the skill cannot map is index-reason-unknown, not silently dropped', () => {
  const f = run('index-reason-unknown', (r) => {
    r.messages.alerts = [{ subject: SUBJECT, examples: [`${ORIGIN}/about/`], reason_label: 'Blocked by a brand new rule' }];
  });
...
test('the alert note: on the noindex remainder, summed across alerts, never on a reason with examples', () => {
  ...
  const named = (n) => ({ subject: SUBJECT, examples: Array.from({ length: n }, (_, i) => `${ORIGIN}/synthetic-page-${i}/`), reason_label: "Excluded by 'noindex' tag" });
  ...
  const own = run('index-reason-noindex', noindex([`${ORIGIN}/about/`], [named(2)]));
...
test('below-floor and videos: severities and wording pinned', () => {
  ...
  const v = run('videos-not-indexed', (r) => { r.videos = { captured_at: r.videos.captured_at, report: 'videos', status: 'ok', indexed: 1, not_indexed: 2 }; });
  assert.deepEqual(v.map((x) => [x.severity, x.detail]), [[INFO, '2 video(s) not indexed, 1 indexed']]);
  assert.ok(!fires('videos-not-indexed', (r) => { r.videos = { captured_at: r.videos.captured_at, report: 'videos', status: 'ok', indexed: 3, not_indexed: 0 }; }));
});

test('a captured issue row fires its check even when the count reads 0', () => {
  const w = run('enhancement-warning', (r) => {
    Object.assign(r.enhancements.items[0], { warning: 0, issues: [{ label: "Missing field 'name'", items: 2, level: 'warning' }] });
  });
  assert.equal(w[0].detail, `0 item(s) with warnings: "Missing field 'name'" (2)`);
  const e = run('enhancement-invalid', (r) => {
    Object.assign(r.enhancements.items[0], { invalid: 0, issues: [{ label: "Missing field 'position'", items: 1, level: 'error' }] });
  });
  assert.equal(e[0].detail, `0 invalid item(s): "Missing field 'position'" (1)`);
});
...
test('capture-run2.json: an early run is known, below the floor, and missing videos', () => {
  const findings = evaluateCapture(readFixture('capture-run2.json'), { ...healthyOpts(), now: PREINDEX_NOW });
  ...
  assert.deepEqual(of('enhancement-warning').map((f) => f.url).sort(), ['Breadcrumbs']);
});
