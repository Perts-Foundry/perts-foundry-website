...
export const PREINDEX_NOW = new Date('2026-09-25T21:00:00Z');
export const ORIGIN = 'https://pertsfoundry.com';
...
// The homepage plus one published service, one case study, one small-business page and one blog
// post, plus the site's other static and hub pages: the same set the healthy capture's performance
// PAGES tab and inspections cover.
export const SITEMAP_URLS = Object.freeze([
  '/', '/about/', '/accessibility/', '/contact/', '/privacy/', '/services/', '/case-studies/', '/blog/',
  '/small-business/', '/services/cloud-infrastructure/', '/services/devsecops-devops/',
  '/case-studies/finops-cloud-cost-savings/', '/case-studies/zero-downtime-registry-migration/',
  '/small-business/website-design/', '/small-business/it-support/', '/blog/infrastructure-as-code/',
  '/blog/finops-125k-cost-savings/',
].map((p) => `${ORIGIN}${p}`));
...
const reason = (slug) => mut((r) => {
  r['indexing-pages'].reasons.push({ reason: slug, source: 'Website', count: 1, examples: [`${ORIGIN}/blog/finops-125k-cost-savings/`] });
...
export const FIXTURE_FOR = Object.freeze({
  ...
  'gsc-property-mismatch': mut((r, o, c) => { c.property = 'sc-domain:other-example.test'; }),
  ...
  'association-missing': mut((r) => { r.associations.services = []; }),
  'association-pending': mut((r) => { r.associations.pending = 1; }),

  'removal-active': mut((r) => { r.removals.temporary_active = 1; r.removals.temporary_urls = [`${ORIGIN}/about/`]; }),
  ...
  'messages-alert-examples': mut((r) => {
    r.messages.alerts = [{ subject: 'New reasons prevent pages from being indexed', examples: [`${ORIGIN}/about/`] }];
  }),
  ...
  'sitemap-child-submitted': mut((r) => {
    r.sitemaps.rows.push({ ...r.sitemaps.rows[0], path: `${ORIGIN}/sitemap_blog_1.xml`, type: 'Sitemap', discovered_pages: 7 });
  }),
  ...
  // The site has no video pages, so the healthy capture's videos report is not-present. Firing this
  // check means replacing the whole report with an ok one: mutating a not-present report does nothing.
  'videos-not-indexed': mut((r) => {
    r.videos = { captured_at: r.videos.captured_at, report: 'videos', status: 'ok', indexed: 0, not_indexed: 1 };
  }),

  'inspect-not-indexed': mut((r) => { r.inspections.items[1].verdict = 'not-on-google'; r.inspections.items[1].indexed = false; }),
  'inspect-canonical-mismatch': mut((r) => { r.inspections.items[1].google_canonical = `${ORIGIN}/services/devsecops-devops/`; }),
  ...
  'inspect-rich-result-missing': mut((r) => { r.inspections.items[1].rich_results = []; }),
  ...
  'enhancement-product-duplicate-evidence': mut((r) => {
    r.enhancements.items.push({ type: 'Product snippets', valid: 4, invalid: 0, warning: 0 });
    r.enhancements.items.push({ type: 'Merchant listings', valid: 4, invalid: 0, warning: 0 });
  }),
  ...
  'perf-low-ctr': mut((r) => {
    r.performance.pages.rows[1] = { url: `${ORIGIN}/services/cloud-infrastructure/`, clicks: 1, impressions: 150, ctr: 0.0067, position: 5 };
  }),
