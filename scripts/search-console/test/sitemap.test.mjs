...
const BASE = 'https://pertsfoundry.com';
...
test('index plus children: 12 distinct URLs, image locs excluded, &amp; decoded', async () => {
  const fetchImpl = routes(healthy());
  const out = await liveSitemapUrls(BASE, { fetchImpl, backoff: [] });
  assert.equal(out.partial, false);
  assert.equal(out.urls.length, 12);
...
test('one child failing is partial, with the other child still collected', async () => {
  ...
  assert.equal(out.urls.length, 6);
});
...
test('a redirect to another host is not trusted', async () => {
  ...
  assert.equal(child.urls.length, 6);
});
...
test('a Hugo-shaped single urlset sitemap.xml is read directly, every URL comes back partial: false', async () => {
  const out = await liveSitemapUrls(BASE, { fetchImpl: routes({ [INDEX]: { body: readText('sitemap.xml') } }), backoff: [] });
  assert.equal(out.partial, false);
  assert.equal(out.urls.length, 17);
  for (const loc of readText('sitemap.xml').match(/<loc>([^<]+)<\/loc>/g).map((m) => m.slice(5, -6))) {
    assert.ok(out.urls.includes(loc), loc);
  }
});
