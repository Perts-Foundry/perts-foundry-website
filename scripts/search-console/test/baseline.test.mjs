...
test('stateDir: override, then XDG_STATE_HOME, then HOME', () => {
  ...
  assert.equal(stateDir({ XDG_STATE_HOME: '/xdg', HOME: '/h' }), path.join('/xdg', 'search-console-pertsfoundry'));
  assert.equal(stateDir({ XDG_STATE_HOME: 'relative', HOME: '/h' }), path.join('/h', '.local', 'state', 'search-console-pertsfoundry'));
  assert.equal(stateDir({ HOME: '/h' }), path.join('/h', '.local', 'state', 'search-console-pertsfoundry'));
  assert.equal(stateDir({ SEARCH_CONSOLE_STATE_DIR: '  ', HOME: '/h' }), path.join('/h', '.local', 'state', 'search-console-pertsfoundry'));
});
