...
test('comma thousands and numeric strings become numbers', () => {
  const { capture, errors } = norm((r) => {
    ...
    r.performance.totals.position = '14.5';
    r['crawl-stats'].requests = '500';
  });
  assert.deepEqual(errors, []);
  assert.equal(capture.reports.performance.totals.impressions, 2000);
  assert.equal(capture.reports.performance.totals.position, 14.5);
...
test('booleans and verdicts from page text', () => {
  const { capture } = norm((r) => {
    ...
    r.inspections.items[1].verdict = 'URL is not on Google';
  ...
  assert.equal(items[1].verdict, 'not-on-google');
...
test('host derivation: sc-domain and URL-prefix properties absolutise relative URLs', () => {
  const domain = norm((r) => { r.performance.pages.rows[0].url = '/about/'; });
  assert.equal(domain.capture.reports.performance.pages.rows[0].url, `${ORIGIN}/about/`);

  const c = baseCapture();
  c.property = 'https://www.example.test/';
  c.reports.links.top_linked_pages[0].url = '/about/';
  const prefix = normaliseCapture(c);
  assert.equal(prefix.capture.reports.links.top_linked_pages[0].url, 'https://www.example.test/about/');
});

test('an absolute URL on a foreign host is rejected; a subdomain of a Domain property is not', () => {
  const foreign = norm((r) => { r.performance.pages.rows[0].url = 'https://other-example.test/x'; });
  ...
  const sub = norm((r) => { r.performance.pages.rows[0].url = 'https://www.pertsfoundry.com/x'; });
  assert.deepEqual(sub.errors, []);

  const canonical = norm((r) => { r.inspections.items[0].google_canonical = 'https://other-example.test/'; });
...
test('query strings and fragments are stripped from URL fields', () => {
  const { capture } = norm((r) => {
    r.performance.pages.rows[0].url = `${ORIGIN}/services/cloud-infrastructure/?variant=123#reviews`;
    r['indexing-pages'].reasons = [{ reason: 'not-found', source: 'Website', count: 1, examples: ['/about/?page=2'] }];
    r.inspections.items[1].user_canonical = `${ORIGIN}/services/cloud-infrastructure/?utm_source=x`;
  });
  assert.equal(capture.reports.performance.pages.rows[0].url, `${ORIGIN}/services/cloud-infrastructure/`);
  assert.equal(capture.reports['indexing-pages'].reasons[0].examples[0], `${ORIGIN}/about/`);
  assert.equal(capture.reports.inspections.items[1].user_canonical, `${ORIGIN}/services/cloud-infrastructure/`);
});
...
test('every reason label, with its punctuation, maps to its slug', () => {
  ...
  for (const [label, slug] of Object.entries(cases)) {
    ...
    const { capture } = norm((r) => {
      r['indexing-pages'].reasons = [{ reason: label, source: 'Website', count: 1, examples: [] }];
    });
...
test('an unknown reason label is kept under unknown with the label preserved', () => {
  const { capture } = norm((r) => {
    ...
    r['indexing-pages'].reasons = [{ reason: 'Blocked by page removal tool', source: 'Website', count: 1, examples: [] }];
  });
...
test('normalise runs before validate: the raw form fails, the normalised form passes', () => {
  const c = baseCapture();
  c.reports.performance.totals.ctr = '3%';
