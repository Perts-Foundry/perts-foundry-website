...
import { importClosure, importsOf } from '../lib/shared/import-closure.mjs';
...
test('accepted-risks shape: a valid entry per subject kind passes', () => {
  const entries = [
    ...
    { check: 'inspect-stale-crawl', path: '/about/', note: 'page', accepted_on: '2026-12-01' },
    { check: 'index-reason-noindex', path: '/blog/finops-125k-cost-savings/', note: 'page-or-reason', accepted_on: '2026-12-01' },
    ...
    { check: 'association-missing', path: 'analytics', note: 'service', accepted_on: '2026-12-01' },
    ...
    { check: 'perf-low-ctr', path: '/services/cloud-infrastructure/', note: 'perf by page', accepted_on: '2026-12-01' },
  ];
...
test('accepted-risks shape: each malformed entry is named', () => {
  const bad = [
    ...
    [{ check: 'inspect-stale-crawl', path: 'about', note: 'x', accepted_on: '2026-12-01' }, /URL path/],
    ...
    [{ check: 'inspect-stale-crawl', path: '/about/?x=1', note: 'x', accepted_on: '2026-12-01' }, /query string/],
  ];
...
test('review.mjs loads only allow-listed modules and builtins, and never node:child_process', async () => {
  ...
  const allowed = (f) => {
    ...
    return rel.startsWith('scripts/search-console/') && !rel.startsWith('scripts/search-console/test/');
  };
