...
site sitemap, runs every check, diffs each report against its latest comparable run, saves the
...
Vendored under `lib/shared/`, holding only the exports this skill uses: `findings.mjs` (severities,
`partitionAccepted`, `diffFindings`, `findingKey`, `exitCodeFor`), `http.mjs` (node fetch, never
curl), `extract.mjs` (sitemap parsers), `display-path.mjs`, and `import-closure.mjs` (test support
only). `test/contract.test.mjs` holds the import closure of `review.mjs` to files under
`scripts/search-console/` and keeps `node:child_process` out of it.
...
property host with no query string or fragment, never under `/api/` (whole segments, anywhere in
the path, any case), and never with a token-shaped segment: 20 or more unbroken letters, digits or
underscores including a digit or a capital, or a UUID. Hyphens break the run, so a content slug
passes; so do long all-lowercase words and a generated `sitemap_<type>_<n>.xml` child name (Hugo's
own build never emits one). A `discovery` view path obeys the same token rule. No
...
(`SUBJECT_KIND`): `page` (a site URL), `report`, `device`, `service`, `host`,
...
| `ai-control-exclude` | WARN | singleton | removes the site from AI Overviews and AI Mode |
| `email-notifications-off` | WARN | singleton | critical issues would arrive unseen |
| `secondary-domain-redirect` | WARN, INFO when a permanent redirect to the canonical host | host | a second brand host must redirect, not serve; every host in `SECONDARY_DOMAINS` (["www.pertsfoundry.com"]) plus any the capture lists |
...
| `association-missing` | INFO | service | no Analytics association |
...
| `enhancement-absent` | INFO | enhancement-type | Breadcrumbs absent while an inspected page matching `EXPECTED_RICH_RESULTS` is indexed; judged only once the Enhancements report exists |
| `enhancement-product-duplicate-evidence` | INFO | singleton | two rich-result families reporting items for the same markup; kept registered for a future overlap, though the site's current rich results do not trigger it |
...
`NOINDEX_OK` is empty: a real Hugo build of this site emits no noindex meta tag anywhere; taxonomy
pages and the 404 page are kept out of the index via robots.txt and sitemap exclusion instead.

## State dir and run files

`SEARCH_CONSOLE_STATE_DIR`, else `$XDG_STATE_HOME/search-console-pertsfoundry`, else
`~/.local/state/search-console-pertsfoundry`. A state dir that resolves inside the worktree or the
primary checkout (lexically, through a symlink either way, or by its nearest existing ancestor) is
refused with exit 2.
