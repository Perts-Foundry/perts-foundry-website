// TEST SUPPORT. The read-only guard the contract test uses to prove review.mjs only loads what it
// is allowed to load, and never node:child_process.
//
// Vendored from the Sapphire Shadow Studio search-console skill's `scripts/lib/import-closure.mjs`;
// only `importClosure` and `importsOf`, the two exports the contract test imports, are kept here.
//
// THE GUARD IS ONLY AS GOOD AS ITS CONTROLS. `importClosure` fails LOUDLY rather than treating
