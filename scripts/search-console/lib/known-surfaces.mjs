...
//
// Seeded from a live walk of pertsfoundry.com's Search Console property on 2026-09-22, the port's
// live check (see the plan's "Live check of the property" section). `conditional: true` marks an
// item that is present only once data exists, so its absence is not reported as `surface-gone`;
// rich-result reports come and go with data, so their removal by Google is deliberately never
// reported.
...
export const KNOWN_SURFACES_REVIEWED_ON = '2026-09-22';
...
export const KNOWN_SURFACES = Object.freeze({
  ...
  nav: Object.freeze([
    ...
    { label: 'HTTPS', path: 'https', conditional: true },
