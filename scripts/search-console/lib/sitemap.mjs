...
// allowlist closes every other host unvisited. The site numbers Search Console is compared against
// come from here, anonymously, with the shared fetch helper.
//
// Never curl: Cloudflare's bot management can block curl's fingerprint and return a hard 429, which
// would read as an unreachable sitemap. Node's fetch is not flagged (lib/shared/http.mjs).

import { fetchPage, BROWSER_HEADERS } from './shared/http.mjs';
import { parseSitemapLocs, parseSitemapChildren } from './shared/extract.mjs';
