...
//
// A finding is { check, severity, url, detail }, the shared finding shape, so the accepted-risks
// matcher and the differ are reused as they are. `url` is the SUBJECT, and SUBJECT_KIND says which
// kind each check uses: a site URL for per-page findings, a report id, a device, an
...
import { ERROR, WARN, INFO, pathOf, findingKey } from './shared/findings.mjs';
import {
...
export const PROPERTY = 'sc-domain:pertsfoundry.com';
export const PROPERTY_HOST = 'pertsfoundry.com';

// Brand queries, compared with spaces removed so "perts foundry" still counts.
export const BRAND_TERMS = Object.freeze(['perts foundry', 'pertsfoundry', 'seth perts']);
...
// Secondary hosts that answer for this site and should redirect to the canonical property host. A
// node probe on the port date (2026-09-22) confirmed `www.pertsfoundry.com` answers 301 to
// `https://pertsfoundry.com/`. It does not change between runs, so it is a constant rather than a
// transcribed field; `review.mjs` probes the union of this list and any host a capture still lists
// in `settings.secondary_domains`.
export const SECONDARY_DOMAINS = Object.freeze(['www.pertsfoundry.com']);
...
// route family). A `hugo --gc --minify --cleanDestinationDir` build (2026-09-22) grepped for
// "noindex" across the whole `public/` tree and found none: the theme emits no robots noindex meta
// anywhere. Taxonomy pages (/tags/, /categories/) and the 404 page are kept out of Google instead via
// robots.txt Disallow and, respectively, by never appearing in the sitemap and by Google's own
// handling of a 404 status; none of that shows up as a noindex meta tag, and none of those paths ever
// appear in `sitemap.xml`, so there is nothing here to exempt today. Kept as a live object rather than
// deleted so a future noindex page has a place to land without a shape change.
...
export const EXPECTED_RICH_RESULTS = Object.freeze([
  Object.freeze({ match: '^/services/[^/]+/$', require: [['Breadcrumbs']] }),
  Object.freeze({ match: '^/case-studies/[^/]+/$', require: [['Breadcrumbs']] }),
  Object.freeze({ match: '^/small-business/[^/]+/$', require: [['Breadcrumbs']] }),
  Object.freeze({ match: '^/blog/[^/]+/$', require: [['Breadcrumbs']] }),
]);
...
function settingsChecks({ capture, okRep, add, opts }) {
  ...
  if (s.verified === false) add('verification-lost', 'verification-lost', 'the Ownership verification page no longer says you are a verified owner');
    ...
    add('verification-method-fragile', 'verification-method-fragile', `verified by a method a site deploy can remove (${safeText(s.method, 120)}); DNS is sturdier`);
  }
  ...
  if (s.ai_control === 'exclude') add('ai-control-exclude', 'ai-control-exclude', 'Search generative AI is set to Exclude; the site is removed from AI Overviews and AI Mode');
...
// Merchant Center is out of scope: the site sells no products, so a missing association there is
// not a finding. GA4 is out of scope for this port too, so only the Analytics association is judged.
function associationChecks({ okRep, add }) {
  const a = okRep('associations');
  if (!a) return;
  for (const svc of ['analytics']) {
    if (!a.services.includes(svc)) add('association-missing', svc, `no ${svc} association`);
  }
...
function enhancementChecks({ reports, okRep, add }) {
  // Only a report that exists is judged. Enhancement types the property has not yet accumulated
  // enough rich-result data for are legitimately absent from the navigation, so an absent section is
  // not absent markup; see the enhancement-absent check below for the direct evidence this skill
  // actually relies on.
  ...
  // No two rich-result types are expected to overlap on this site (it has no Merchant Center or
  // review-app integration), so `enhancement-product-duplicate-evidence` never fires today. The check
  // id stays registered for accepted-risks and the contract test; it is generic enough to catch a
  // future overlap (for example, two rich-result families both reporting items for the same markup).
  ...
  if (has(types, 'Product snippets') && has(types, 'Merchant listings')) {
    add('enhancement-product-duplicate-evidence', 'enhancement-product-duplicate-evidence',
      'Product snippets and Merchant listings both report items; two rich-result families overlap on the same markup, check for a duplicate JSON-LD emitter');
  }
  const ins = okRep('inspections');
  const richResultIndexed = ins?.items.some((i) => {
    const p = pathOf(i.url);
    return EXPECTED_RICH_RESULTS.some((r) => new RegExp(r.match).test(p)) && i.verdict === 'on-google' && i.indexed;
  });
  if (richResultIndexed && !has(types, 'Breadcrumbs')) {
    add('enhancement-absent', 'Breadcrumbs', 'absent while an inspected URL matching an EXPECTED_RICH_RESULTS rule is indexed');
  }
}
