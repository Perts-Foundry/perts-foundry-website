...
// This site's non-page routes: the Worker API, whose URLs never belong in a capture. Matched as
// whole path segments anywhere in the path, case-insensitively, the same shape as the customer and
// token routes this constant replaces.
export const FORBIDDEN_ROUTES = Object.freeze(['api']);
...
// Hyphens break the run, so a content slug passes, and so does a long all-lowercase word. A generated
// child sitemap name (`sitemap_pages_1.xml`) would be public and exempt by shape; this site's Hugo
// build never emits one, but the exemption is kept for a future multi-file sitemap. A UUID is
...
/** A site page URL: on the property host, no query, no fragment, no token-shaped path. */
